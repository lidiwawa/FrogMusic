...
import {
    refreshNeteaseRecommendations,
    removeLegacyNeteaseRecommendationSheets,
...
export default async function () {
    ...
    void removeLegacyNeteaseRecommendationSheets().catch((error) => {
        console.info(
            "[netease-recommendation] legacy sheet cleanup skipped:",
            error?.message ?? error,
        );
    });

    void refreshNeteaseRecommendations()
        .then((recommendations) => {
            console.info(
                `[netease-recommendation] startup refreshed: ${recommendations.length}`,
            );
        })
        .catch((error) => {
            console.info(
                "[netease-recommendation] startup refresh skipped:",
                error?.message ?? error,
            );
        });
}
