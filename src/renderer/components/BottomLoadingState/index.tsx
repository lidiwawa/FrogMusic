...
export default function BottomLoadingState(props: IProps) {
    const { state, onLoadMore, autoLoadMore = false } = props;
    ...
    useEffect(() => {
        const intersectionObserver = new IntersectionObserver((entries) => {
            if (
                (autoLoadMore || AppConfig.getConfig("normal.autoLoadMore")) &&
    ...
    if (state === RequestStateCode.FINISHED) {
        ...
        component = autoLoadMore || AppConfig.getConfig("normal.autoLoadMore")
            ? <>
                <div className="lds-dual-ring"></div> {t("bottom_loading_state.loading")}
            </>
            : <span className="bottom-loading-state--loadmore" role="button" onClick={onLoadMore}>
                {t("bottom_loading_state.load_more")}
            </span>;
    } else if (state === RequestStateCode.ERROR) {
        component = <span
            className="bottom-loading-state--loadmore"
            role="button"
            onClick={onLoadMore}
        >
            加载失败，点击重试
