...
function seekToLyricTime(seconds: number) {
    trackPlayer.seekTo(Math.max(0, seconds));
    trackPlayer.resume();
}
                        ...
                        >
                            {lyricParser?.getLyricItems?.()?.map((lyricItem, index) => (
                                ...
                                >
                                    ...
                                    <button
                                        type="button"
                                        className="lyric-seek-button"
                                        title={`${t("common.music_state_play")} ${formatLyricTime(lyricItem.time)}`}
                                        aria-label={`${t("common.music_state_play")} ${formatLyricTime(lyricItem.time)}`}
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            seekToLyricTime(lyricItem.time);
                                        }}
                                    >
                                        <SvgAsset iconName="play" size={10}></SvgAsset>
                                        <span>{formatLyricTime(lyricItem.time)}</span>
                                    </button>
                                </div>
                            ))}
                        </Condition>
