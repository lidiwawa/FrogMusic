    ...
    async function onBackupClick() {
        ...
        try {
            if (
                ...
                if (!(await client.exists("/FrogMusic"))) {
                    await client.createDirectory("/FrogMusic");
                }
                // 临时文件
                await client.putFileContents(
                    "/FrogMusic/FrogMusicBackup.json",
                    backUp,
                    {
                        overwrite: true,
                    },
    ...
    async function onResumeClick() {
        ...
        try {
            if (
                ...
                if (!(await client.exists("/FrogMusic/FrogMusicBackup.json"))) {
                    throw new Error(
                ...
                <div
                    ...
                    onClick={async () => {
                        const result = await dialogUtil.showSaveDialog({
                            ...
                            filters: [
                                {
                                    name: t("settings.backup.frogmusic_backup_file"),
                                    extensions: ["json", "txt"],
                                },
                            ],
